...
func TestHasSliceNotApplied(t *testing.T) {
	snap := pb.Snapshot{
	...
	for i, tt := range tests {
		...
		hasNext := raftLog.HasSliceNotApplied()
...
func TestSliceNotApplied(t *testing.T) {
	snap := pb.Snapshot{
		Metadata: &pb.SnapshotMetadata{Term: 1, Index: 3},
	}
	ents := []pb.Entry{
...
func TestLogRestore(t *testing.T) {
	...
	if len(raftLog.SliceNotApplied()) != 0 {
		t.Errorf("len = %d, want 0", len(raftLog.SliceNotApplied()))
	}
